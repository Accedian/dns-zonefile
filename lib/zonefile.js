...
let parse = function (text, hostname, auth) {


  text = removeComments(text);
  text = flatten(text);
  retText = parseRRs(text)

  if (hostname) {

    let arecs = parseRRs(text).a
    const axios = require('axios');
    const path = '/api/v2/cyber/network-assets'

    const baseURL = hostname + path;

    let processedIndex = 0;

    let putReqs = buildBatch(arecs, processedIndex, processedIndex + 100, baseURL, axios, auth)

    sendBatch(putReqs, axios, processedIndex, baseURL, arecs, auth)
...
let buildBatch = function(arecs, startIndex, endIndex, baseURL, axios, auth) {
  var putReqs= new Array();
  for (index = startIndex; index < endIndex; index++) {  
    //let putX = axios.put(baseURL+arecs[index].id, arecs[index]);
    let putX = axios.post(baseURL, {
      data: arecs[index]
    }, {
      headers: {
...
let sendBatch = function(putReqs, axios, processedIndex, baseURL, arecs,auth) {
  axios.all(putReqs).then(axios.spread((...responses) => {
    for (ind=0; ind<responses.length; ind++) {
      console.log(`\n Batch ${processedIndex}`)
      // console.log(`Response ${responses[ind].status} ${responses[ind].data.result} ${responses[ind].data._id}`)
      console.log(`Response ${responses[ind].status} ${responses[ind].statusText} ${responses[ind].data.data.id}`)
    }
    processedIndex=processedIndex+responses.length
    if (processedIndex < arecs.length) {
      let maxForBatch = processedIndex+100
      if (arecs.length < maxForBatch){
        maxForBatch=arecs.length
      }
      let nputReqs = buildBatch(arecs,processedIndex,maxForBatch,baseURL,axios,auth)
      sendBatch(nputReqs, axios, processedIndex, baseURL,arecs,auth)
    }
  })).catch(errors => {
    // react on errors.
  })
...
let parseRRs = function (text) {
  ...
  for (let rr of rrs) {
    ...
    //   ret.ns.push(parseNS(normalizeRR(rr, 'NS'), ret.ns));
      ...
      ret.a = ret.a || [];
      let parsedArec = parseA(normalizeRR(rr, 'A'), ret.a)
      

      if (parsedArec.attributes && parsedArec.attributes.meta.dnsname) {
        //let genES = genESJSON(parsedArec)
        //ret.a.push(genES);
        ret.a.push(parsedArec);
      }
    }
...
let genESJSON = function(arec) {
  let index = '893edffa-cd60-4709-ac43-846db1cb4c72_network_assets'
  let type = '_doc'
  let result = {
     create: {
       _index: index,
